...
use app\modules\api\models\FootstepAddForm;
...
class DefaultController extends Controller
{
    ...
    public function actionIndex()
    {
       ...
       //var_dump($form->search()[data][cat_list][0][goods_list]);die;
        return $form->search();
    }
    ...
    public function actionGoods()
    {
        ...
        //添加到足迹
        $form = new FootstepAddForm();
        $form->attributes = \Yii::$app->request->get();
        if (!\Yii::$app->user->isGuest) {
            $form->user_id = \Yii::$app->user->id;
        }
        $form->store_id = $this->store->id;
        $form->goods_id = $data['data']['id'];
        $footstep= new BaseApiResponse($form->save());
        return $data;
    }
